...
export function FlipCard({ data }: FlipCardProps) {
  ...
  return (
    ...
    >
      {/* FRONT: Profile */}
      <motion.div
        className="absolute inset-0 backface-hidden rounded-md border-2 border-foreground/20 px-4 py-6 flex flex-col items-center justify-center bg-gradient-to-br from-muted via-primary to-muted text-center"
      ...
      >
        <img
          src={data.image}
          alt={data.name}
          className="size-50 md:size-70 rounded-xl object-cover mb-4 border-2 bg-accent/50 border-foreground/20"
        />
      ...
      <motion.div
        className="absolute inset-0 backface-hidden rounded-md border-2 border-foreground/20 px-4 py-6 flex flex-col justify-between items-center gap-y-4 bg-gradient-to-tr from-muted via-primary to-muted "
        initial={{ rotateY: 180 }}
